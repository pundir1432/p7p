const referralModel = require("../model/referral.model");
...
const applyCoupon = async (req, res) => {
  try {
    ...
    // console.log(coupon,"hit===");
    if (coupon.usedCount >= coupon.count) {
      return res.status(400).json({
          error: "Coupon code has been used maximum times and is Expired",
        });
    }
    ...
    if (typeof coupon.usedCount === "number" && !isNaN(coupon.usedCount)) {
      coupon.usedCount += 1;
    } else {
      coupon.usedCount = 1;
    }
    ...
    //     const sharingUser = await userModel.findById(req.body.sharedBy);
    //     if (sharingUser) {
    //         sharingUser.points += 50;
